...
DB_URL = "..\\library_database.pickle"
...
class Library:
    ...
    def add_customer(self, customer_id: int, customer_first_name: str, customer_last_name: str, address: Address,
                     email: str, birth_date):
            ...
            for key in loan:
                ...
                if int(customer_id) == int(customer_id) and status == "Loaned":
                    ...
                    if datetime.date.today() > return_date_for_change:
                        status = "Expired"
                    print(
                        f"| Book: [{book_id}] `{book.get_book_name()}` by {book_author} | Loaned: {loan_date} Return: {return_date} | Status: {status}")
    ...
    def get_loan_logs(self):
        ...
        for key in loans:
            ...
            if datetime.date.today() > return_date_for_change:
                status = "Expired"
            print(
                f"| Customer: [{customer_id}] {customer_name}. | Book: [{book_id}] `{book_name}` By Author: {book_author}. | Loan Date: {loan_date}, Return Date: {return_date} | Status: {status}")
    ...
    if library_menu_choose == "5":
        try:
            if os.path.isfile(DB_URL):
                ...
                print("Bye!")
            exit()  # Note: Don't forget add a save system here!
...
def goto_customer_menu():
    ...
    if customer_menu_choose == "1":
        try:
            ...
            time.sleep(2)
            library.add_customer(id_of_customer, first_name_of_customer, last_name_of_customer, address_of_customer,
                                 email_of_customer, birthdate_of_customer)
